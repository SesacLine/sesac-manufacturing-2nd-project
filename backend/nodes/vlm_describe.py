...
관측의 세 생산자:
  CNN 라벨        -> group["pattern"] 그대로 (① cnn.py — ResNet-18 실판정 및 실연동. 체크포인트 없으면 "Center" 폴백)
  die-matrix 통계 -> 실연동: 멤버 규칙(_member_keys — ① 판정이 그룹 패턴과 일치하는
                     웨이퍼만)으로 거른 die_map을 stacking으로 겹쳐 스택맵을 만들고
                     wafer_reading.quantitative가 shape/zone/signature/angular/clock/density/
                     continuity/defect_die_ratio를 KG 어휘로 계산한다.
  VLM 자연어      -> VLM_LIVE=1이면 실연동(wafer_reading.vlm.VLMReader):
                     location/morphology_text에 더해 total_description/vlm_track/image_mode를
                     관측에 싣는다. 기본(미설정)은 빈 값 — signature가 있으면 KG는 enum
                     진입이라 자연어가 없어도 된다. total_description은 사용자 최종 응답에
                     노출될 서술이다(배선은 backend 담당자와 협의 중).
...
def _member_keys(
    pattern: str, lot_ids: list[str], cnn_results: list[dict]
) -> list[tuple[str, str]]:
    """스태킹 멤버 규칙: ① 판정 pattern이 그룹 pattern과 일치하는 웨이퍼만.

    is_normal 등 fab.db 라벨로 거르면 안됨(정답 누출) — 반드시 ① CNN 판정으로.
    """
...
def _build_observation(
    pattern: str, lot_ids: list[str], cnn_results: list[dict] | None = None
) -> Observation:
    ...
    cluster@center). cnn_results가 없는 단독 호출은 종전대로 로트 전체.
    """
    keys = _member_keys(pattern, lot_ids, cnn_results or [])
        ...
        groups = [
            {
                **group,
                "observation": _build_observation(
                    group["pattern"], group["lot_ids"], state.get("cnn_results")
                ),
            }
