"""설계 B + angular_coverage 판별자 — 관측 모폴로지로 후보를 재가중한다.
...
def rerank_by_observation(candidates: list[dict], observation: dict | None) -> list[dict]:
    ...
    return sorted(candidates, key=lambda c: c["morphology_score"], reverse=True)
