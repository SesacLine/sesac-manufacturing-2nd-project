...
| ④ | `nodes/hypothesis.py` | MCP 실호출로 증거 수집 + 근거 리치 보존 |
| ⑤ | `nodes/critic.py` | 4규칙 채택/기각 + 고정 사유 토큰(P2~P5) 부여 |
...
- LLM을 실시간 호출하는 노드는 **①VLM과 ⑦응답생성 둘뿐**(2026-07-09 결정). 지금은 둘 다
  하드코딩/템플릿이다. ③~⑤는 원래부터 결정적 룰베이스 함수이지 LLM이 아니다.
...
- 배치 진행 로그를 남기는 `LoggingMCP`(`batch_runner.py`)는 이 싱글턴을 감싸는 **위임 프록시**일
  뿐 세션을 새로 만들지 않는다.
- `hypothesis.py`의 `(step, evidence_label, evidence)` 캐싱 키도 같은 이유로 유지해야 한다.
...
| # | 위치 | 지금 선택 | 비고 |
|---|---|---|---|
| 1 | `hypothesis.py` — MCP 호출 단위 | `(step, evidence_label, evidence)` 캐싱 | 캐싱 없이는 244건에서 타임아웃 실측. 응급 처치 수준 |
| 2 | `hypothesis.py` — `step=null` 후보 | `step=None`을 그대로 commonality에 전달 | 전체 공정이 뭉뚱그려져 신호가 흐려짐 |
| 3 | `hypothesis.py` — `direction: null`인 `[자동]` | 방향 무시, 정상범위 이탈이면 drift | 4건뿐이라 영향 작음 |
...
| 8 | `critic.py` — `semi_auto` 판정 | 잠정 자동 기각 | 사람 판정 경로(명세 §4-2)가 생기면 제거 |
