...
# (구 AC-9 `test_group_node_functions_keep_old_signature_via_adapter`는 #33 step 3에서 삭제됐다.
#  "④~⑦이 옛 시그니처를 유지한다"는 주장 자체가 뒤집혔기 때문이다 — 어댑터를 걷어내고 노드가
#  GroupState를 직접 받는다. 대체 검증은 test_skeleton_flatten.py의 AC-1~5.)
...
def test_subgraph_unmapped_path_routes_to_respond_without_llm():
    """④ 후보 0건 → route_on_candidates → ⑦' respond 통합 경로를 실제 ainvoke로 탄다.

    ④ 후보 0건 처리는 build/critic을 건너뛰므로 mcp는 안 불린다(object() 더미로 충분). 순수함수
    단위 테스트(test_skeleton_response)가 우회했던 '진입점 + 조건부 라우팅 + 종단' 통합을 덮는다.
    (#33 평탄화 후: 서브그래프 노드가 GroupState를 직접 받으므로 어댑터 fake-state 재구성은 없다.)
    """
    import asyncio
