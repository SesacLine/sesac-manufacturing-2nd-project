...
from .nodes import critic, describer, graphrag, grouper, hypothesis, lowyield, response, vlm
...
def build_graph(kg_client: KGClient, mcp: MCPClient):
    ...
    # ③ VLM description 생성(v1.5: VLM은 Grouper 뒤, 스택맵에 1회) — 스켈레톤은 패턴별 결정적 템플릿.
    workflow.add_node("describe_groups", describer.describe_groups)
    ...
    workflow.add_edge("group_by_pattern", "describe_groups")
    workflow.add_edge("describe_groups", "fetch_graphrag_candidates")
