...
WM-811K 웨이퍼맵과 SECS/GEM 시뮬레이터 기반 합성 공정·장비 데이터를
...
생성 규모(기본 시드 20260101): 가상 팹 90일치 — 배경 lot 800개 + 시나리오 lot 84개,
텔레메트리 60,100행, 정비 84건, 알람 136건, 시나리오 11개(단일 원인 9 + 매칭불가 2).
...
(MixedWM38은 팀 결정으로 제외됨 — 원천은 WM-811K 단일. `preprocess/mw38_loader.py`는 미사용 잔재.)
...
## 설치
    ...
    pip install -e . && pip install pytest pyyaml

## 데이터 준비 (원천 데이터는 배포에 미포함)

    # WM-811K — MIR Lab 공식 배포본 (원천 데이터는 이것 하나뿐 — MixedWM38은 팀 결정으로 제외)
