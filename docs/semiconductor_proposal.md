...
### GraphRAG 기반 웨이퍼맵 결함 근본 원인 설명 multi-agent 시스템

VLM 기반 웨이퍼맵 결함 판독 결과를 시작점으로, GraphRAG와 Fab 운영 데이터를 결합한 멀티에이전트 추론을 통해 인스턴스 수준의 설명 가능한 Root Cause Analysis(RCA)를 수행하는 시스템.결함은 공정, 장비, 공정 파라미터 등 다양한 요인의 상호작용으로 발생하므로, **온톨로지 기반 그래프를 활용해 원인 후보를 탐색**하고, **Hypothesis Agent와 Critic Agent가 실제 Fab 데이터를 근거로 원인 가설을 생성·검증**한다.

</aside>

- 핵심 기능
    - WM-811K 공개 데이터와 도메인 문헌만으로, 웨이퍼맵 이미지를 입력받아 결함 패턴을 판독한다.
    - Hypothesis 노드가 GraphRAG(`kg_rca`)가 미리 결정적으로 순회해 찾아둔 공정 수준 원인 후보마다 검증등급(`[자동]`/`[반자동]`/`[근거없음]`)에 따라 정해진 MCP 도구를 호출해 증거를 모으고, Critic 노드가 시간 정합성·반대 근거·근거 정합성(faithfulness)을 규칙으로 점검해 채택·기각하거나 근거가 부족하면 즉시 "판단 불가"를 반환한다.
...
- **기존 방식의 한계**
    ...
    - 이 때문에 결함을 서술하고 설명까지 하는 VLM(Vision Language Model) 기반 접근이 대안으로 부상하고 있다.
- **현업의 문제**
...
- **멀티 에이전트 시스템의 필요성**
    ...
    - Hypothesis 노드는 "무엇을 더 조사할까"(탐색)를 맡아, GraphRAG가 빌드타임에 이미 결정적으로 찾아둔 복수 가설마다 검증등급(`[자동]`/`[반자동]`/`[근거없음]`)에 따라 정해진 MCP 도구를 호출해 증거를 모은다.
    - Critic 노드는 "이 답이 근거에 맞나"(검증)를 맡아 시간 정합성·반대 근거·faithfulness를 규칙으로 판정하고, 근거가 부족하면 재시도 없이 즉시 "판단 불가"를 반환한다.
    - 이 탐색과 검증의 역할 분리가 본 시스템의 핵심이며, 둘 다 결정적 룰베이스 노드로 구현해 판단 근거를 항상 추적 가능하게 한다.
    - 벡터 RAG는 "무엇이 무엇을 유발했는가"라는 다중 홉 관계 추론에 취약하므로, 판독 결과를 조건으로 검색을 좁히는 지식그래프 검색이 두 노드의 공유 도구가 된다.
- *참고 자료 URL 첨부*
    - https://developer.nvidia.com/blog/optimizing-semiconductor-defect-classification-with-generative-ai-and-vision-foundation-models/ (NVIDIA, 2025)
    - https://arxiv.org/html/2604.27629v4 (WaferSAGE, 2026)
    - https://elec4.co.kr/contents/article_detail?article_idx=37189 (AI 기반 검사 기술 동향, 2026)
    ...
    - https://news.skhynix.co.kr/seominsuk-column-test/ (반도체 검사/테스트, SK하이닉스)
...
| --- | --- | --- | --- |
| 1 | GraphRAG기반 원인 후보 생성 (도구) | 판독된 결함 패턴과 지식그래프를 기반으로 가능한 공정 원인 후보를 생성한다.
*→ 지식그래프는 VLM 출력 결과인 결함 유형, 형상, 공간 등을 시작 노드로 정의하고, 이와 연결 될 결함 메커니즘 (공정, 공정별 변수)을 문서 기반으로 구축* | 1. 최소구현 |
| 2 | Hypothesis 노드 (가설 검증) | GraphRAG(`kg_rca`)가 결정적으로 미리 순회해 둔 원인 후보마다, 검증등급(`[자동]`/`[반자동]`/`[근거없음]`)에 따라 정해진 MCP Tool(Commonality·Lot History·Telemetry·Alarm·Maintenance 등, 9종)을 조건부로 호출해 증거를 수집하고 가설별 Evidence를 구성한다. 결정적 함수(룰베이스)로 구현하며 LLM은 쓰지 않는다. | 1. 최소구현 |
| 3 | Critic 노드 (검증) | Hypothesis 결과를 대상으로 시간 정합성, 정상 Lot 비교, 인과성, 증거 부족 여부를 결정적 함수(룰베이스)로 검증하여 가설을 채택·기각하거나, 채택 가능한 후보가 없으면 재시도 없이 즉시 "판단 불가(근거부족)"를 반환한다. | 1. 최소구현 |
| 4 | 웨이퍼맵 결함 패턴 판독 (도구) | 웨이퍼맵 이미지에서 결함 패턴(Center, Edge-Ring 등)을 판독하고 공간/형상을 자연어로 서술한다.
*→ WM 데이터 내 9가지 결함 유형 전체를 판독 대상으로 하되, GraphRAG~응답생성까지 끝까지 연결되는 건 Center/Edge-Ring/Scratch 3종뿐이다. 나머지 6종은 판독까지만 지원하고 "원인 분석 데이터 없음"으로 명시한다.* | 1. 최소구현 |
...
| WaferSAGE |   • 소형 VLM(Qwen3-VL-4B) 기반 웨이퍼 결함 VQA. 합성 데이터 생성과 루브릭 기반 강화학습으로 데이터 희소성 극복, 온프레미스 배포. | https://arxiv.org/html/2604.27629v4 |
| NVIDIA VLM/VFM 결함 분류 |   • Vision Language Model(Cosmos Reason)과 Vision Foundation Model(NV-DINOv2)로 결함 분류
  • 설명가능성과 auto-labeling으로 유사 케이스 구분 | NVIDIA Developer Blog (2025) https://developer.nvidia.com/blog/optimizing-semiconductor-defect-classification-with-generative-ai-and-vision-foundation-models/ |
                ...
                - 과정:
                    - IMG → Text: Gemini 3 Flash을 활용해 웨이퍼 맵의 공간적 특징과 근본 원인을 설명하는 텍스트를 생성
                        - Gemini 3 Flash 사용 이유
                            ...
                            - 2. 루브릭 생성을 위한 '기준점(Teacher Model)' 역할
...
**GraphRAG와 Fab 데이터를 결합한 Root Cause Analysis**

- 문헌 기반 원인 후보(GraphRAG)와 실제 Fab 운영 데이터(Lot History, Telemetry, Alarm, Maintenance)를 함께 활용하여 근본 원인을 검증한다.
...
- Hypothesis 노드가 GraphRAG(`kg_rca`)가 이미 결정적으로 찾아둔 복수 가설 각각에 검증등급별 증거를 수집하고, Critic 노드가 근거 정합성을 규칙으로 검증하는 구조로, 경로를 한 번 찾고 끝나는 단일 파이프라인과 차별화된다.
- **환각 억제**:
...
- 결함 패턴(Center, Edge-Ring, Scratch 등) 판독 및 VLM 입력 데이터로 활용

**6.2 GraphRAG 지식그래프** 

- 반도체 제조 교재, 공정 매뉴얼, 결함 분석 논문 등 신뢰성 있는 도메인 문헌을 활용한다.
- RCA에 필요한 공정, 장비, 결함 패턴, 공정 파라미터, 원인 및 영향 관계를 중심으로 온톨로지를 정의하고, LLM으로 엔티티·관계를 1차 자동 추출해 스키마에 맞게 구조화한다.
- 그래프 스키마: `DefectPattern(고정 3종: Center/Scratch/Edge-Ring) → ProcessStep(고정 6종) → FailureMode → Cause → Evidence(:Parameter|:Maintenance|:Recipe)`. `DefectPattern`/`ProcessStep`/`Parameter`만 고정 vocabulary로 매핑하고 `FailureMode`/`Cause`/`Maintenance`/`Recipe`는 문헌에서 LLM이 자유 추출한다.
...
| 논문 | Wafer defect semantic reasoning via a three-stage retrieval-augmented
system in semiconductor manufacturing (논문) | 웨이퍼맵과 자연어 질의를 함께 입력받아 결함 유형뿐 아니라 공정 원인과 역할별(품질·공정·AI 엔지니어) 설명까지 생성하는 GraphRAG 계열 선행 연구. KG 추출 프롬프트 설계와 사례조사(§5)에 참조. | Liao, X., Zhang, J., Lyu, Y., & Wang, J. (2026). Wafer defect semantic reasoning via a three-stage retrieval-augmented system in semiconductor manufacturing. *Journal of Manufacturing Systems*, *85*, 269-286. |
...
| --- | --- | --- |
| VLM | 웨이퍼맵 결함 판독
*(실제 fine-tuning을 진행하는 것을 목표로 하되, 차선으로 few-shot 기반 구현을 병렬적으로 진행 할 예정)* | 웨이퍼맵을 파인튜닝된 VLM으로 판독.
(백본 VLM은 Qwen3-VL-4B 나 8B-Thinking 사용 예정)
 |
| Neo4j | 관계 기반 그래프 DB | 결함-공정-장비-원인 관계로 저장해 다중 홉 추론과 근거 경로 제시, Root Cause 후보를 생성 |
| GraphRAG | 그래프 기반 지식검색 | 결함 패턴과 연관된 원인 후보를 검색하여 Hypothesis 노드에 전달 |
...
| LangGraph | 파이프라인 오케스트레이션 | ⓪~⑥ 노드(VLM·Grouper·GraphRAG·Hypothesis·Critic·응답생성)의 실행 흐름과 상태(RCAState)를 관리 |
| LLM | 엔티티/관계 추출 및 설명 생성 | KG 구축(오프라인)의 문헌 추출과 최종 응답생성의 문장 합성에 사용 |
...
⓪ 저수율 로트 선별 (wafer.die_map 직접 집계, 결정적 함수)
        │
        ▼
① VLM (결함 패턴 판독) — 실시간 LLM 호출
        │
        ▼
② Grouper (패턴별 그룹화, 결정적 함수)
        │
        ▼
③ GraphRAG — kg_rca가 빌드타임에 이미 결정적으로 순회해 둔
   DefectPattern→ProcessStep→FailureMode→Cause→Evidence 경로 조회
        │
        ▼
④ Hypothesis 노드 (결정적 함수)
   candidate의 검증등급(tier)에 따라 MCP 도구 조건부 호출:
...
- ①VLM과 ⑥응답생성만 LLM을 실시간 호출하는 단발 노드이고, ③GraphRAG는 빌드타임에 이미 끝난 결과를 조회만 한다.
- ④Hypothesis가 MCP 도구를 부르는 것도 candidate의 검증등급(tier, KG가 빌드타임에 이미 부여)에 따른 조건 분기다.
- 채택 후보가 0개면 재시도 없이 즉시 `insufficient_evidence`를 반환한다.

#### 7.2 Hypothesis Workflow

```
**입력: defect_pattern(VLM 판독), lot_ids(불량 lot들)**

[1] 가설 생성 candidates = kg_candidate_causes(defect_pattern)   ← 서버 밖(kg_rca 그래프 순회). 가설은 서버가 안 만듦

[2] 공통 (P1) comm = mcp.run_commonality_analysis(lot_ids)       ← tier 무관, 모든 candidate 공통
각 후보 cand마다:
    suspect = top_equipment_for(comm, cand.step)
    [3] 지지 (P2)  cand.tier로 분기:
...
**반환: evidence (`[자동]` 등급 외엔 최종 판정은 안 함)**
```
...
- 액션 아이템
    ...
    - Hypothesis 노드 구현 (GraphRAG 원인 후보의 검증등급별 MCP Tool 조건부 호출로 증거 수집)
    - Critic 노드 구현 (시간 정합성, 정상 Lot 비교, 인과성 검증 및 가설 기각/판단불가 처리)
...
- 대략적 타임라인
    ...
    | 3주차 | 판독 모델 베이스라인, Hypothesis 노드 1차 구현, Critic 노드와 LangGraph 파이프라인 통합, 응답 생성 |
...
| faithfulness | 생성 설명의 근거 정합률과 "판단 불가" 반환의 적정성 | https://arxiv.org/html/2604.27629v4 |
| 경로 정합성 | 결함에서 원인, 공정, 근거로 이어지는 추론 경로의 정합 여부 | Journal of Manufacturing Systems 85 (2026) |
| 단일경로 vs 다중가설탐색 RCA 품질 | 동일 결함 그룹에서 Hypothesis 노드가 복수 후보를 병렬 탐색·검증할 때 다중 원인/모호 케이스 정확도, 경로 정합성, 기각된 오답 가설 수를 단일 경로(첫 후보만 채택) 대비 비교 | https://arxiv.org/html/2604.27629v4 |
| 사용자 만족도 | 수율 엔지니어 대상 5점 Likert(명확성, 유용성), 결함 유형별 | Journal of Manufacturing Systems 85 (2026) |
| KG-Fab 어휘 정합성 | GraphRAG가 추출한 cause 어휘와 fab 합성 데이터 ground truth 어휘 간 정합률 — top-k accuracy 계산의 선결 조건 | — |

---

## 참고문헌

[1] Spotfire, "Accelerating yield improvement: Root cause analysis in semiconductor manufacturing," 2025. https://www.spotfire.com/blog/2025/11/18/accelerating-yield-improvement-root-cause-analysis-in-semiconductor-manufacturing/

[2] Frandzzo Global, "Semiconductor Manufacturing Stability — Yield & Cost Control," 2026. https://www.frandzzo.com/newsroom/semiconductor-manufacturing-stability

[3] PDF Solutions, "Predicting Yield Loss from Source: Machine Learning for Compound Semiconductor Manufacturing," 2025. https://www.pdf.com/predicting-yield-loss-from-source-machine-learning-for-compound-semiconductor-manufacturing/

[4] Qult Technologies, "An ML Solution to Identify the Root Cause of Yield Loss in Semiconductor Manufacturing," 2024. https://www.qult.ai/an-ml-solution-to-identify-the-root-cause-of-yield-loss-in-semiconductor-manufacturing-2/

[5] H. Razouk et al., "AI-Based Knowledge Management System for Risk Assessment and Root Cause Analysis in Semiconductor Industry," in AI for Digitising Industry – Applications, Ch.11, 2022. https://www.taylorfrancis.com/chapters/oa-edit/10.1201/9781003337232-11/

[6] D. Edge et al., "From Local to Global: A GraphRAG Approach to Query-Focused Summarization," Microsoft Research, 2024. (개요: IBM, "What is GraphRAG?", 2026. https://www.ibm.com/think/topics/graphrag)

[7] "RAG vs. GraphRAG: A Systematic Evaluation and Key Insights," arXiv:2502.11371, 2025. https://arxiv.org/html/2502.11371v2

[8] "Interactive and Intelligent Root Cause Analysis in Manufacturing with Causal Bayesian Networks and Knowledge Graphs," arXiv:2402.00043, 2024. https://arxiv.org/pdf/2402.00043

[9] "Combining informed data-driven anomaly detection with knowledge graphs for root cause analysis in predictive maintenance," Engineering Applications of AI, 2025. https://www.sciencedirect.com/science/article/pii/S0952197625001526

[10] MathWorks, "Classify Defects on Wafer Maps Using Deep Learning (WM-811K)." https://www.mathworks.com/help/vision/ug/classify-defects-on-wafer-maps-using-deep-learning.html

[11] "WaferSAGE: LLM-Powered Wafer Defect Analysis via Synthetic Data Generation and Rubric-Guided Reinforcement Learning," arXiv:2604.27629, 2026. https://arxiv.org/abs/2604.27629

[12] FriendliAI, "Automating Industrial Inspection with Vision Language Models," 2026. https://friendli.ai/blog/industrial-inspection-with-vision-language-models
...
| Hypothesis(가설) 노드 | 복수의 원인 가설(GraphRAG가 빌드타임에 이미 결정적으로 찾아 둔 경로)에 대해, 검증등급(`[자동]`/`[반자동]`/`[근거없음]`)에 따라 정해진 MCP 도구를 조건부로 호출해 증거를 모으는 결정적 함수 노드 |
| Critic(비평) 노드 | 생성 답변이 근거에 정합하는지(faithfulness)와 경로가 정합적인지 규칙으로 판정하고, 근거가 부족하면 재시도 없이 즉시 "판단 불가"를 반환하는 결정적 함수 노드 |
