...
| *→ 본 프로젝트가 다루는 결함 패턴은 Center/Edge-Ring/Scratch 3종뿐이며(§6.1), CNN은 이 3종에 `Unknown`·`Normal`을 더해 판정한다. WM-811K의 나머지 결함 패턴은 전부 `Unknown`("새로운 결함 패턴")으로 처리한다. `Normal`은 정상 웨이퍼이므로 그룹을 만들지 않는다.* |  |  |  |
...
- 9개 패턴(Center, Donut, Edge-Loc, Edge-Ring, Loc, Near-Full, Scratch, Random, None)을 포함하고 있으나, 본 프로젝트에서는 그 중에서도 가장 기본적인 결함 패턴인 "Center", "Edge-Ring", "Scratch"만 다룬다. 나머지 중 결함 패턴 5개(Donut, Edge-Loc, Loc, Near-Full, Random)는 "새로운 결함 패턴"(`Unknown`)으로 가정하고, `None`은 결함이 아닌 정상 웨이퍼이므로 `Normal`로 판정해 그룹을 만들지 않는다.
...
| LangGraph | 파이프라인 오케스트레이션 | ⓪~⑦ 노드(저수율 로트 선별·CNN·Grouper·VLM·KG조회·Hypothesis·Critic·응답생성)의 실행 흐름과 상태(RCAState)를 관리 |
...
⓪ 저수율 로트 선별 (wafer.die_map 직접 집계, 결정적 함수)
        │
        ▼
① CNN (결함 패턴 판정) — 실시간 비전 모델 추론
   Center / Edge-Ring / Scratch 3종 + Unknown + Normal
        │
        ▼
② Grouper (CNN 판정 결과 기반 유사 패턴 그룹화, 결정적 함수)
   Normal은 정상 웨이퍼이므로 그룹 미생성
        │
        ▼
