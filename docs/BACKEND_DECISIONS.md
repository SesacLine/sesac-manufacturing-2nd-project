...
| # | 결정 | 근거 / 되돌릴 조건 |
|---|---|---|
| D1 | **대표 원인(`hypotheses[0]`) = accepted 중 kg_rca 후보 순위 최상위** (hypotheses.json `rank` = candidates 배열 순서. Critic이 순서를 보존하므로 `accepted[0]`). 비채택 후보는 accepted 뒤에 원순서로 잇고, 정렬 확정 후 `h{n}` 부여 | 명세 §4-1 미결정 — kg_rca 점수체계(evidence_docs/chunks 기반)가 이미 후보 순위를 내므로 그 순위를 재사용. ⑦에 LLM이 붙거나 팀이 다른 규칙을 정하면 `nodes/response.py`만 교체 |
