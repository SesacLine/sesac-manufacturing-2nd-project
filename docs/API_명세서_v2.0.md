...
> **버전** `v2.0` · **작성일** 2026-07-25 (v1.0: 2026-07-16)
> **대상 시스템** React+Vite 프론트엔드 ↔ FastAPI 백엔드 (REST)
> **근거 문서**: `기획안_v1.5.md`, 와이어프레임 v8(`frontend/rca_wireframe_final8.html`)

> 변경 이력·설계 논의 과정은 별도 문서 `API_CHANGELOG.md` 참조. 직전 판은 `API_명세서_v1.0.md`.

> **v2.0 변경 요약** (전부 additive — v1.0 키 변경·삭제 없음):
> 1. `status`에 **`novel`**(미지 패턴 OSR) 신설 — 구 `unmapped`에서 분리(2.2·2.5). `pattern:"Unknown"` 그룹은 이제 `novel`.
> 2. 2.2 items·2.5 응답에 **`confidence`**(R1 확신 수준 `medium|low` — `high` 없음) 노출.
> 3. 2.2 items·2.5 응답에 **`yield_impact`**(그룹 수율영향 %p, 계산식 확정) 추가.
> 4. 2.5 응답에 그룹 단위 **`actions`**(권장 조치 `{type, hold, text}` 구조화) 추가.
...
> - 이 고정값은 서버 설정 한 곳(`backend/config.py`의 `EVENT_DATE`)에 두고, **env `EVENT_DATE=2026-05-01`처럼 오버라이드할 수 있다**(.env — 서버시간을 바꿔 테스트하는 장치. 데이터가 2026-01~03월분이라 벽시계 now()는 항상 안 맞는다). 실데이터 전환 시 `now()`로 되돌린다. 날짜에 `datetime.now()` 사용 금지가 코드 규약이다(시각 HH:MM:SS 부분만 허용).
...
```json
{
  ...
  "yield_impact": -3.8,
  "actions": [
    { "type": "containment",   "hold": true,  "text": "영향 로트 8건 Hold·격리 여부 검토 (수율영향 기반 disposition)" },
    { "type": "corrective",    "hold": false, "text": "ETCH-01 점검 — etch_nonuniformity 관련 부위·파라미터 확인" },
    { "type": "preventive",    "hold": false, "text": "재발 방지 — 관련 알람 임계·PM 주기 재검토" }
  ],
  "lot_count": 8,
...
- **`verdict` (Critic 판정)**: `accepted`(채택) | `rejected`(기각) | `judge_unknown`(미판정) — 비채택 사유는 `verdict_reason`이 담는다. (그룹 `status`의 `insufficient`와는 다른 층 — verdict는 가설 1건, status는 그룹 전체.)
  - **`verdict_reason` (비채택 사유, Nullable)**: `accepted`이면 `null`, `rejected`/`judge_unknown`이면 문자열. 값은 기획안 v1.5 Critic Workflow(§7.2) 규칙에 대응한다 — `rejected`는 ①시간 선후(P2)·②반대근거(P3, `get_normal_lot_ratio`)·③faithfulness(P4)·**④인과 서명 없음(P6, 0726 신설)**, `judge_unknown`은 ⑤KG 메커니즘 연결 없음(P5) + 미조사(반자동 SEMI_AUTO_PENDING / 자동 tier의 조사 실패 폴백 NOT_INVESTIGATED).
    - **`P6_NO_CAUSAL_SIGNATURE`(0726 신설)**: 채택 직전 게이트 — 불량 로트 **전부**가 그 장비를 거쳤고(`commonality_ratio == 1.0`) 그 파라미터가 **KG 예상 방향으로** 이탈했는지(`direction_match is True`)를 함께 요구한다. 상관(모든 로트가 지나감)만으로는 채택하지 않는다는 뜻이며, **미지 결함 배치가 `status:"insufficient"`로 떨어지게 만드는 규칙**이다(정본 `docs/node_langraph_spec/node_spec_06_critic.md` §4-5). `judge_unknown`이 **아니라** `rejected`다 — "안 봤다"가 아니라 적극적 반대 증거이기 때문.
...
```json
{
  ...
  "actions": [
    { "type": "containment",   "hold": true,  "text": "영향 로트 8건 Hold·격리 여부 검토 (수율영향 기반 disposition)" },
    { "type": "investigation", "hold": false, "text": "수동 조사 이관 — 담당 엔지니어 배정" },
    { "type": "investigation", "hold": false, "text": "후보 장비 정상 로트 표본 확대·텔레메트리 보강 후 재분석" }
  ],
  "lot_count": 8,
  "lot_ids": ["lot30112", "lot30988", "lot31544", "lot32077", "lot32610", "lot33291", "lot33845", "lot34120"],
  "hypotheses": [ { "hypothesis_id": "h0", "cause": "clean_nozzle_clog", "stage": "CLEAN", "tier": "auto", "verdict": "judge_unknown", "verdict_reason": "KG 메커니즘 연결(VERIFIED_BY) 없음", "cluster_id": null, "is_primary": false, "narrative": "...", "next_actions": [], "citations": [{ "id": 7, "text": "Lee et al., Microelectronics Reliability 2021" }] } ]
}
```

**응답 200 (미확인 신규 패턴 · `status: "novel"` — v2.0 신설)** — CNN이 `Unknown`(미지 패턴, open-set)으로 판정한 그룹. 억지 분류 대신 abstain하며, **이상 자체는 유효**하므로 격리+전문가 재판독 대상이다. `hypotheses: []`이고, 판독은 됐으므로 `lot_count`/`lot_ids`는 채워진다.
```json
{
  ...
  "status": "novel",
  "reason": "알려진 패턴 어디에도 맞지 않는 새 시각 패턴(open-set) 후보입니다. 억지 분류 대신 판단 불가로 처리하며, 이상은 유효하므로 격리·재판독 대상입니다.",
  "confidence": "low",
  "yield_impact": -1.5,
  "actions": [
    { "type": "containment",   "hold": true,  "text": "영향 로트 6건 Hold·격리 여부 검토 (수율영향 기반 disposition)" },
    { "type": "investigation", "hold": false, "text": "전문가 재판독 — 신규 패턴 여부 확정" },
    { "type": "investigation", "hold": false, "text": "신규 패턴 후보 등록 — 데이터 축적 후 재학습 검토" }
  ],
  "lot_count": 6,
...
```
- `404` — `unmapped`/`novel`(v2.0) 그룹에 대해 근거를 요청한 경우. 가설이 없으므로 근거도 없다.
```json
...
```json
{
  "days":   [ { "date": "2026-01-13", "yield": 91.4 }, { "date": "2026-01-14", "yield": 90.8 } ],
  "events": [ { "date": "2026-03-11", "analysis_id": "grp_center_20260401_01",
                "pattern": "Center", "status": "reviewed", "cause": "clean_nozzle_clog",
                "equipment": "CLEAN-01", "stage": "CLEAN", "tier": "auto",
                "confidence": "medium" } ]
}
```
...
**에러**
- `500` — 이벤트 저장소(`app_state.db`) 조회 실패. `{ "detail": "분석 이벤트를 불러오지 못했습니다." }`

---

### 2.9 `GET /stats/causes` — 장비·원인·패턴 집계 (화면1 도넛/Pareto/원인 칩, v2.0 신설)
완료된 분석의 채택 원인 구성(도넛·칩)과 CNN 판독 패턴 분포(Pareto). 전부 `app_state.db` 저장분 집계라 fab.db가 없어도 동작한다.
...
```json
{
  "equipment": [ { "equipment_id": "CLEAN-01", "stage": "CLEAN", "count": 1 } ],
  "causes":    [ { "pattern": "Center", "cause": "clean_nozzle_clog",
                   "stage": "CLEAN", "tier": "auto", "count": 1 } ],
  "patterns":  [ { "pattern": "Center", "wafer_count": 132, "mapped": true },
                 { "pattern": "Unknown", "wafer_count": 41, "mapped": false } ]
}
```
- **`equipment[]`** (도넛): `status:"reviewed"` 분석들의 **대표(top) 가설 의심 장비** 집계. `count` 내림차순. `stage`는 색 매핑(공정별 색)용 — Nullable.
- **`causes[]`** (패턴별 채택 원인 칩): reviewed 분석들의 top 가설 `(pattern, cause)` 집계. `count`는 반복 횟수(칩의 ×n). `cause`는 열린 문자열(2.2 `top_cause`와 동일 규약 — 하드코딩 금지).
- **`patterns[]`** (Pareto): **CNN 판독 저장분(`wafer_reading`) 집계** — 시뮬레이터 정답 라벨이 아니라 우리 판독 결과라 **정답 누출이 없다**. `mapped` = KG 매핑 대상 3종(`Center`/`Edge-Ring`/`Scratch`) 여부(파랑/회색 구분용).
- **빈 상태**: 분석·판독이 아직 없으면 세 배열 모두 `[]`(200 — 형태 계약). 집계는 조회 시점 계산이지만 원천이 저장분이라 결정적이다.

**에러**
- `500` — 저장소 조회 실패. `{ "detail": "원인 통계를 불러오지 못했습니다." }`
...
| AnalysisSummary | analysis_id, pattern, lot_count, top_cause, status, confidence, yield_impact | `app_state.db` |
| Batch | batch_id, status, current_step, steps[], logs[], result_ids[] | `app_state.db` (checkpoint) |
| Analysis | analysis_id, pattern, description, status, reason, confidence, yield_impact, actions[], hypotheses[], lot_ids[] | Neo4j + `app_state.db` |
| Hypothesis | hypothesis_id, cause, stage, tier, verdict, verdict_reason, cluster_id, is_primary, narrative, next_actions[], citations[] | Neo4j |
| YieldDaily (2.8) | days[] (`date`, `yield`), events[] (`date`, `analysis_id`, `pattern`, `status`, `cause`, `equipment`, `stage`, `tier`, `confidence`) | `fab.db` + `app_state.db` |
| CauseStats (2.9) | equipment[], causes[], patterns[] (`mapped`) | `app_state.db` |
...
> - `confidence` (R1 확신 수준, v2.0): `medium`(잠정 지지) | `low`(불확실) — **`high`(확정) 없음**
> - `actions[].type` (권장 조치 분류, v2.0): `containment` | `corrective` | `preventive` | `investigation` — 한국어 라벨은 프론트 소유
...
> **핵심 원칙**
> 1. **"결과 없음"은 에러가 아니다** — 빈 목록·판단불가(`insufficient`)·매핑없음(`unmapped`)·신규 패턴(`novel`)은 모두 `200`으로 정상 반환한다.
...
- **fallback**: `hypotheses[0]`가 없거나(`unmapped`/`novel`/`insufficient`) `stage`가 `null`이면 공정 조각을 "원인 매핑 없음"(`unmapped`)·"신규 패턴"(`novel`)·"판단 불가"(`insufficient`)·"공정 미상"(`stage` 부재) 고정 문구로 대체한다. `stage=null`은 실측상 순위 꼬리(전체 6~9%)라 채택 대표 `hypotheses[0]`엔 사실상 안 나타나므로 "공정 미상"은 거의 안 쓰인다.
...
**형상 gloss 상수표 (KG 표준 엔티티 5종)** — 입력은 `pattern`. 원인 매핑 3종(Center/Edge-Ring/Scratch)만 `reviewed`/`insufficient` 가능, `Unknown`은 `novel`(v2.0).
...
| `Unknown` | 미지/새로운 결함 패턴(원 9종 중 Edge-Loc·Loc·Donut·Near-Full·Random 통합) | ✗ novel(v2.0) |
