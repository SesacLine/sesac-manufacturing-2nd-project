...
> **계약 정본: `docs/API_명세서_v2.0.md`** (v1.0은 이력 보존용 — 정본 아님). 이 문서의 모든 `§n`은 그 명세서의 절 번호다. 절은 Grep으로 `### 2.7` 형태로 찾아 그 절만 읽는다(행번호로 찾지 마라).
...
**라우터는 `backend/api/` 하위 모듈로 분리**하고 `main.py`는 앱 조립(CORS·의존성·라우터 등록)만 한다. 현재 `main.py`에 엔드포인트가 직접 박혀 있으나 8종(§3.1 표 기준)으로 늘리면서 이 구조로 옮긴다.
