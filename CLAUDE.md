...
- **현재 상태(2026-07-13)**: 파이프라인 ⓪~⑥ **Walking Skeleton 완성**, end-to-end 실행 검증됨.
  단, VLM(①)과 응답생성(⑥)은 실제 LLM 미연동 상태(하드코딩/템플릿) — 아래 "단순화 목록" 참고.
...
`GET /health`. 프론트엔드용 목표 API(7종, 미확정 사항 포함)는 `docs/API_명세서_v0.1.md` 별도 설계 문서.
...
- 그래프 스키마(v2.3~v2.4, 정본 `kg_rca/schema_v2.md`): 노드 8종
  ...
  `DefectPattern`(3종: Center/Edge-Ring/Scratch)·`ProcessStep`(6종)·`Parameter`(20종)만
  고정 vocabulary, 나머지는 LLM이 문헌에서 자유 추출.
...
- 출력물: `kg_rca/outputs/hypotheses.json` — backend가 읽는 유일한 산출물(Neo4j 없이도 이 파일만
  있으면 backend 동작 가능). 가설 수는 재생성마다 바뀌므로 코드에 하드코딩 금지.
- 실행 스크립트는 `0_reset.py`~`6_ask_graphrag.py` 순서(번호가 실행 순서).
- 알려진 미해결 문제(정본 `kg_rca/STATUS.md` §4): 검증신호 Maintenance 쏠림(P1), 가설 점수체계
  타당성 없음(P2, LLM 자기평가 기반이라 재설계 필요), Maintenance 노드 중복(P3), 추출 비결정성(P4),
  CLEAN/EDS 공정 문헌 공백(P5), ProcessStep join 의미필터 없음(P6).

## secsgem-mcp (MCP 서버, 9종 도구)

WM-811K/MixedWM38 웨이퍼맵 + SECS/GEM 시뮬레이터 합성 fab 데이터(`fab.db`, SQLite, read-only)를
...
(WM-811K/MixedWM38 원천 데이터 다운로드 후 `python -m simulator.generate`로 빌드).
...
| 프론트엔드용 목표 API 설계(7종, 미확정 사항 있는 초안) | `docs/API_명세서_v0.1.md` |
| KG 스키마 전체 명세(정본) | `kg_rca/schema_v2.md` |
