...
④ 증거 수집·검증·재랭킹  자동 tier LLM 그룹 조사관 + MCP 9종 도구  ← "이번에 실제로 무슨 일이"
⑤ 검증             시간정합·반대증거·faithfulness·KG메커니즘 4규칙 + 미조사 보류
⑥ 응답 생성         대표 원인(fab 재랭킹 최상위) 정렬 → 카드 조립
```
...
- **④ Hypothesis·⑤ Critic 갱신(2026-07-23)**: 스켈레톤을 넘어 ④ 자동 tier LLM 그룹 조사관·방향
  대조·클러스터·fab 재랭킹·스텝 상한, ⑤ 4규칙 + 미조사 보류까지 구현. **fab.db 기반 E2E 평가로
  SC-CENTER-01 근본원인 top-1 달성**(정답 193위 rejected → top-1 accepted, 함정 P2 44건 기각).
  상세는 `backend/README.md`·`docs/BACKEND_DECISIONS.md`(D13~D16)·`personalspace_rca`의 ④/⑤ 문서.

### 2026-07-20 스냅샷 (아래는 당시 기록 — 위 0723 갱신이 우선)
